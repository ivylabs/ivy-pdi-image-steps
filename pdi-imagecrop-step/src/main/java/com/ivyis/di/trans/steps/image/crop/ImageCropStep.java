...
public class ImageCropStep extends BaseStep implements StepInterface {
    ...
    public ImageCropStep(StepMeta s, StepDataInterface stepDataInterface, int c, TransMeta t, Trans dis) {
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        ...
        try {
            ...
            if (log.isDetailed()) {
                logDetailed("New File Path: '" + fileToPath + "', Size - Width: " + ip.getWidth() + ", Height: "
                        + ip.getHeight());
            }
    ...
    private void cachePosition() throws KettleStepException {
        ...
        if (meta.getAxisX() != null && data.indexOfAxisXField < 0) {
            ...
            if (data.indexOfAxisXField < 0) {
                final String message = "Unable to find table name field [" + meta.getAxisX() + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
        ...
        if (meta.getAxisY() != null && data.indexOfAxisYField < 0) {
            ...
            if (data.indexOfAxisYField < 0) {
                final String message = "Unable to find table name field [" + meta.getAxisY() + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
        ...
        if (meta.getTargetWidth() != null && data.indexOfTargetWidthField < 0) {
            ...
            if (data.indexOfTargetWidthField < 0) {
                final String message = "Unable to find table name field [" + meta.getTargetWidth() + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
        ...
        if (meta.getTargetHeight() != null && data.indexOfTargetHeightField < 0) {
            ...
            if (data.indexOfTargetHeightField < 0) {
                final String message = "Unable to find table name field [" + meta.getTargetHeight() + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
